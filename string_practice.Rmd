...
alarm_report <- data.frame(
  stringsAsFactors = FALSE,
                      date = c("2020-03-14",
                               "2020-03-15","2020-04-10","2020-04-18"),
                  building = c("Engineering-North","Bren Hall","Engineering-South",
                               "Engr-North"),
...
alarm_report_update <- alarm_report %>%
  ...
  mutate(wing = building) %>% 
  
  mutate(wing = str_replace(wing, pattern = "Engineering-", replacement = "")) %>% 
  
  mutate(wing = str_replace(wing, pattern = "Bren Hall", replacement = "NA")) %>% 
